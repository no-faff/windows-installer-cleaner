# v1.0 redesign — full implementation plan
...
✅ Task 1 — commit 32c79e1
✅ Task 2 — commit f9bf40b
✅ Task 3 — commit 3a47c8e
✅ Task 4 — commit 0011284
✅ Task 5 — commit 4554154
✅ Task 6 — commit f1dca2c
✅ Task 7 — commit 25c5e87
✅ Task 8 + 12 — commit 0893159 (folded together — App.xaml.cs fully wired)
✅ Task 9 — commit 7ebeb49
✅ Task 10 — commit 4f33eaa
✅ Task 11 — commit e7250c2
✅ Task 13 — commit 768e5b5
✅ Task 14 — commit 3fcd07f
✅ Task 15 — commit 7d798d4
✅ Task 16 — commit ff88941
...
- Drop Phase 2 (component scan) entirely — Phase 1 is sufficient
- Exclusion filters (not warnings) for Adobe/problematic files
- Compact summary main window (~700×300), not a file list
...
src/SimpleWindowsInstallerCleaner/
  Interop/MsiConstants.cs, MsiNativeMethods.cs     ← P/Invoke (will be modified)
  Models/RegisteredPackage.cs, OrphanedFile.cs      ← records (will be modified)
  Services/                                         ← all service interfaces + implementations
  ViewModels/MainViewModel.cs, OrphanedFileViewModel.cs  ← (will be rewritten)
  MainWindow.xaml, MainWindow.xaml.cs               ← (will be rewritten)
  App.xaml, App.xaml.cs                             ← composition root (will be modified)
src/SimpleWindowsInstallerCleaner.Tests/            ← 7 passing tests (will be modified)
```
...
**Why:** Phase 2 is O(components × products) and takes 3+ minutes. It's unnecessary — Phase 1 catches all registrations that matter.
...
1. **InstallerQueryService.cs** — Delete all Phase 2 code:
   - Delete the entire Phase 2 section in `GetRegisteredPackagesCore()` (lines 96–121: the comment block, `EnumerateComponentPathsInInstallerFolder` call, and the foreach loop)
   - Delete `EnumerateComponentPathsInInstallerFolder()` method (lines 282–334)
   - Delete `GetComponentSid()` method (lines 340–370)
   - Delete `GetComponentPath()` method (lines 376–409)
   - Delete `FindOwningProduct()` method (lines 415–459)
   - Remove the `InstallerFolderPrefix` field (lines 32–34) — only used by Phase 2
   - Update the final progress report to not mention "Scan complete" differently

2. **MsiNativeMethods.cs** — Delete:
   - `MsiEnumComponentsEx` declaration (lines 171–179)
   - `MsiGetComponentPathEx` declaration (lines 199–206)
   - `InstallStateLocal` and `InstallStateSource` constants (lines 212–213)

3. **MsiConstants.cs** — Delete:
   - `MsiComponentClient` static class (lines 67–77) — only used by Phase 2

4. **RegisteredPackage.cs** — Remove `IsAdobeWarning` parameter:
   ```csharp
...
5. **OrphanedFile.cs** — Remove `IsAdobeWarning` parameter:
   ```csharp
...
6. **InstallerQueryService.cs** — Update all `new RegisteredPackage(...)` calls to remove the `IsAdobeWarning` argument.

7. **FileSystemScanService.cs** — Update `new OrphanedFile(...)` call to remove `IsAdobeWarning: false`.

8. **FileSystemScanServiceTests.cs** — Update all `RegisteredPackage` and `OrphanedFile` constructor calls to remove `IsAdobeWarning`. Delete the `FindOrphanedFilesAsync_adobe_warning_propagates_from_registered_set` test (it tested Phase 2 behaviour).

**Verify:** `dotnet test src/SimpleWindowsInstallerCleaner.Tests` — all remaining tests pass.
...
**Tests:**
1. `Load_returns_defaults_when_no_file_exists` — new service with nonexistent path → default settings with "Adobe", "Acrobat" filters
2. `Save_then_Load_round_trips` — save custom settings, load, verify all fields match
3. `Load_returns_defaults_when_file_is_corrupt` — write garbage to file, load → defaults
...
The filter logic: for each orphaned file, check if the filename contains any of the filter strings (case-insensitive). If yes → excluded. If no → actionable.
...
**Tests:**
1. `ApplyFilters_separates_matching_files` — files with "Adobe" in name go to Excluded
2. `ApplyFilters_is_case_insensitive` — "adobe" filter matches "ADOBE_PATCH.msp"
3. `ApplyFilters_with_no_filters_returns_all_as_actionable`
4. `ApplyFilters_with_multiple_filters` — "Adobe" and "Acrobat" both match their respective files
5. `ApplyFilters_no_match_returns_all_actionable` — files that don't match any filter
...
No unit tests for this one — it reads real registry state. Testability is via the interface (mock in VM tests).
...
**DeleteFilesService.cs** — Same pattern as `MoveFilesService`. `File.Delete()` in a `Task.Run` loop, catch exceptions per-file.

**Tests** (same pattern as MoveFilesServiceTests, using temp files):
1. `DeleteFilesAsync_deletes_file` — file gone, no errors
2. `DeleteFilesAsync_reports_error_for_missing_source` — nonexistent file → error
...
This is the trickiest task. The approach uses `MsiGetSummaryInformation` and `MsiSummaryInfoGetProperty` from msi.dll — consistent with the existing P/Invoke pattern. No raw COM interop needed.

**Files to create/modify:**
- Modify: `src/SimpleWindowsInstallerCleaner/Interop/MsiNativeMethods.cs` — add new P/Invoke declarations
...
**Important note for Sonnet:** The `MsiGetSummaryInformation` first call with `bufferLen = 0` may return `ERROR_MORE_DATA` (234) rather than `ERROR_SUCCESS`. Handle both — check `if (error != MsiError.Success && error != MsiError.MoreData) return string.Empty;` in `GetStringProperty`, same as the existing double-call pattern elsewhere in the codebase.

**No unit tests for this service** — it reads real MSI files. Testability is via the interface.
...
**IFileSystemScanService.cs** — change return type:
```csharp
...
**FileSystemScanService.cs** — the method now also counts registered files on disk:
...
The `internal` test constructor with `overrideFiles` stays — it still works, the test just needs to be updated for the new return type.
...
- Add a test for registered file counting (mock returns 2 registered, 1 orphaned → verify `RegisteredFileCount == 2`)
  - Note: this test can't verify `RegisteredTotalBytes` easily since the files don't exist on disk in tests. That's fine — just verify the count is 0 (files don't exist) and the orphan list is correct.
...
**Files to modify:**
- `src/SimpleWindowsInstallerCleaner/ViewModels/MainViewModel.cs` — full rewrite
- `src/SimpleWindowsInstallerCleaner/ViewModels/OrphanedFileViewModel.cs` — keep as-is (used in details window)
- `src/SimpleWindowsInstallerCleaner/App.xaml.cs` — update composition root
...
1. `ScanAsync()` — auto-invoked on startup:
   - Check pending reboot (if setting enabled) → set `HasPendingReboot`
...
2. `BrowseDestination()` — opens `OpenFolderDialog`, sets `MoveDestination`, saves settings

3. `MoveAllAsync()` — moves all actionable (non-excluded) orphaned files:
   - Uses the actionable list from `_lastFilteredResult`
   - Shows progress
   - On completion, re-runs scan to refresh counts

4. `DeleteAllAsync()` — deletes all actionable orphaned files:
...
5. `OpenOrphanedDetails()` — opens the orphaned files detail window (Task 10)

6. `OpenSettings()` — opens settings dialog (Task 11)

7. `RefreshAsync()` — re-runs scan (same as ScanAsync, but can be triggered by user)
...
**Helper method for size formatting** — extract to a static helper since it's used in multiple places:
```csharp
...
**Files to modify:**
- `src/SimpleWindowsInstallerCleaner/MainWindow.xaml` — full rewrite
- `src/SimpleWindowsInstallerCleaner/MainWindow.xaml.cs` — minimal changes
...
Note: `ResizeMode="CanMinimize"` — the compact summary doesn't benefit from resizing. PatchCleaner did the same.
...
├─────────────────────────────────────────────────────────────┤
│                                                             │
│   (pending reboot warning banner — only when applicable)    │
│                                                             │
│   ● {count} files still used                    {size}      │
│   ● {count} files excluded by filters           {size}      │
│   ● {count} files orphaned                      {size}  ›   │
│                                                             │
│  ┌───────────────────────────────────────┐ ┌──────────────┐│
│  │ {move destination path}               │ │ Browse...    ││
│  └───────────────────────────────────────┘ └──────────────┘│
│                                                             │
│  ┌──────────┐                              ┌──────────────┐│
│  │   Move   │                              │    Delete    ││
│  └──────────┘                              └──────────────┘│
│                                                             │
│  ⚙ Settings                      ♥ Donate   no-faff.github │
└─────────────────────────────────────────────────────────────┘
```
...
2. **Three summary lines:** Use a `StackPanel` with three rows. Each row is a `DockPanel`:
   - Left: bullet (●) + descriptive text
   - Right: size display, right-aligned
   - The orphaned line has an additional `Button` styled as a text link ("details..." or "›") that opens the details window
...
4. **Move button:** Accent colour (e.g. `#0078D4` — Windows blue). White text. Prominent.

5. **Delete button:** Neutral/subtle styling. Not red (too alarming for a valid action) — use default button style but right-aligned. The confirmation dialog provides the safety.

6. **Bottom bar:** Left: "⚙ Settings" button (text-style). Right: website link.
...
**Empty/initial state:** Before first scan completes, the three summary lines show "—" for counts and sizes. The scanning overlay appears immediately on startup.
...
- `string SelectedSummary` — e.g. "12 selected (847.2 MB)"
- Commands: `SelectAll`, `SelectNone`

**Window layout (~600×450):**
```
┌─ Orphaned files ────────────────────────────────── ×─┐
│                                                       │
...
- Store the selection state so that if the user opens details, deselects some, closes, then clicks Move — only the selected ones are moved

Actually — simpler approach: the **details window is informational** for v1.0. Move/Delete on the main window always operates on all actionable (non-excluded) files. The details window shows what will be affected. Per-file deselection is a nice-to-have but adds significant complexity (syncing state between windows). **Skip per-file selection for v1.0.** The details window is read-only — it shows what's orphaned and what's excluded. This matches PatchCleaner's details window which was also read-only.
...
│  AcrobatPatch.msp           2.4 MB    .msp            │
│                                                       │
│  3 orphaned (45.1 MB) · 1 excluded (2.4 MB)          │
│                                          [Close]      │
└───────────────────────────────────────────────────────┘
```
...
**SettingsViewModel:**
- `ObservableCollection<string> Filters` — current exclusion filters
- `string NewFilter` — text input for adding a filter
- `bool CheckPendingReboot` — toggle
...
**Window layout (~450×350):**
```
┌─ Settings ──────────────────────────────────── ×─┐
│                                                   │
│  Exclusion filters                                │
│  Files matching these terms won't be              │
│  moved or deleted.                                │
│                                                   │
│  ┌──────────────────────────┐  ┌─────┐            │
│  │                          │  │ Add │            │
...
- The [×] buttons remove individual filters
...
**Why:** PatchCleaner shows "86 files still used, 2.36 GB — details..." The details view shows registered products.
...
**Note:** This window shows the registered packages from the last scan result. The data is already available from `IInstallerQueryService` — the `RegisteredPackage` records have `ProductName`, `ProductCode`, and `LocalPackagePath`.
...
**Window layout (~650×450):**
```
┌─ Registered files ─────────────────────────────── ×─┐
│                                                      │
│  Product name              File             Size     │
│  ─────────────────────────────────────────────────   │
│  Microsoft Office 365      abc123.msi      45.2 MB   │
│  Visual Studio 2022        def456.msi      12.8 MB   │
│  Adobe Acrobat DC          patch1.msp       8.1 MB   │
│  ...                                                 │
│                                                      │
│  86 registered files (2.36 GB)                       │
│                                         [Close]      │
└──────────────────────────────────────────────────────┘
```
...
- In `RegisteredFilesWindow`: same — click a file to see its metadata.

**Layout update for OrphanedFilesWindow (~750×450, wider to accommodate details panel):**
```
┌─ Orphaned files ──────────────────────────────────────── ×─┐
│                                                             │
│  filename.msi          14.2 MB  │  Author: Microsoft Corp   │
│  patch123.msp ◄────     8.1 MB  │  Title: Office Update     │
│  update.msi            22.8 MB  │  Subject: Security patch   │
│                                 │  Comment: KB5001234        │
│  ─── Excluded ──────            │  Signature: Microsoft Corp │
│  AcrobatPatch.msp       2.4 MB  │                            │
│                                                             │
│  3 orphaned (45.1 MB) · 1 excluded (2.4 MB)       [Close]  │
└─────────────────────────────────────────────────────────────┘
```
...
1. **Window icon** — Create or source a simple icon for the app. Add `<ApplicationIcon>` to csproj if you have an .ico file. If no icon is available, skip — it can be added later.

2. **About/info** — Add an info button to the main window bottom bar. Clicking it shows a simple message box:
   - "Simple Windows installer cleaner v1.0.0"
   - "Part of the No faff suite"
   - "MIT licence · github.com/no-faff"

3. **Donate link** — Add "♥ Donate" or similar to the bottom bar. Opens a URL in the default browser. Leave the URL as a placeholder (e.g. `https://github.com/no-faff/windows-installer-cleaner`) until the actual donate link is set up.

4. **Error handling for non-admin** — In `App.xaml.cs`, wrap the startup in a try-catch. If the scan fails with an access-denied error, show a MessageBox: "This app requires administrator privileges. Please right-click and choose 'Run as administrator'."

5. **Move destination placeholder** — When `MoveDestination` is empty, the TextBox should show watermark text "Choose a destination folder..." in grey italic.

6. **Delete confirmation** — Ensure the delete confirmation dialog uses `MessageBox.Show` with `YesNo` buttons and `Warning` icon. The message should read: "Permanently delete {count} files ({size})?\n\nThis cannot be undone."

7. **Move success message** — After move completes, show a brief status. After delete completes, show a brief status. These can be a transient status text on the main window (no separate dialog needed).

8. **Refresh** — The scan command should be re-executable. Add a small refresh button (🔄 or just "Refresh") near the summary lines. This just re-invokes `ScanCommand`.

9. **Keyboard shortcuts** — `F5` = Refresh/Scan. Standard window shortcuts (Alt+F4 etc.) handled by WPF automatically.

10. **British English check** — scan all user-facing strings. Must be British English, sentence case. Examples: "colour" not "color", "organise" not "organize", "cancelled" not "canceled". However, the technical terms are fine as-is (e.g. "orphaned" is correct in both).

11. **Framework-dependent publish** — Add a publish profile or document the command:
    ```
...
**Verify:**
- `dotnet test src/SimpleWindowsInstallerCleaner.Tests --verbosity normal` — all tests pass
- `dotnet build src/SimpleWindowsInstallerCleaner -c Release` — 0 errors, 0 warnings
...
**Why:** The existing `FileSystemScanServiceTests` need updating for the new `ScanResult` return type and the removal of `IsAdobeWarning`. Some existing tests may have broken during earlier tasks — this task catches any stragglers.

**Check and fix:**
1. All `RegisteredPackage` constructors — 3 args now, not 4
2. All `OrphanedFile` constructors — 3 args now, not 4
3. All references to `FindOrphanedFilesAsync` → `ScanAsync`
4. All assertions against `IsAdobeWarning` → removed
5. The `Registered()` test helper → updated
...
src/SimpleWindowsInstallerCleaner/
  Interop/
    MsiConstants.cs              ← modified (removed MsiComponentClient, added MsiSummaryProperty, VtType)
    MsiNativeMethods.cs          ← modified (removed component APIs, added summary info APIs)
  Models/
    AppSettings.cs               ← NEW
    MsiSummaryInfo.cs            ← NEW
    OrphanedFile.cs              ← modified (removed IsAdobeWarning)
    RegisteredPackage.cs         ← modified (removed IsAdobeWarning)
    ScanResult.cs                ← NEW
  Services/
    IInstallerQueryService.cs    ← unchanged
    InstallerQueryService.cs     ← modified (Phase 2 removed)
    IFileSystemScanService.cs    ← modified (returns ScanResult)
    FileSystemScanService.cs     ← modified (returns ScanResult)
    IMoveFilesService.cs         ← unchanged
    MoveFilesService.cs          ← unchanged
    IDeleteFilesService.cs       ← NEW
    DeleteFilesService.cs        ← NEW
    IExclusionService.cs         ← NEW
    ExclusionService.cs          ← NEW
    ISettingsService.cs          ← NEW
    SettingsService.cs           ← NEW
    IPendingRebootService.cs     ← NEW
    PendingRebootService.cs      ← NEW
    IMsiFileInfoService.cs       ← NEW (interface in separate file or combined — up to you)
    MsiFileInfoService.cs        ← NEW
  ViewModels/
    MainViewModel.cs             ← rewritten
    OrphanedFileViewModel.cs     ← kept (may be used later for per-file selection)
    OrphanedFilesViewModel.cs    ← NEW (for details window)
    SettingsViewModel.cs         ← NEW
  MainWindow.xaml                ← rewritten
  MainWindow.xaml.cs             ← minimal changes
  OrphanedFilesWindow.xaml       ← NEW
  OrphanedFilesWindow.xaml.cs    ← NEW
  RegisteredFilesWindow.xaml     ← NEW
  RegisteredFilesWindow.xaml.cs  ← NEW
  SettingsWindow.xaml            ← NEW
  SettingsWindow.xaml.cs         ← NEW
  App.xaml                       ← unchanged
  App.xaml.cs                    ← modified (new composition root)
  app.manifest                   ← unchanged
  GlobalUsings.cs                ← unchanged

src/SimpleWindowsInstallerCleaner.Tests/
  Services/
    FileSystemScanServiceTests.cs   ← modified
    MoveFilesServiceTests.cs        ← unchanged
    ExclusionServiceTests.cs        ← NEW
    SettingsServiceTests.cs         ← NEW
    DeleteFilesServiceTests.cs      ← NEW
  PlaceholderTest.cs                ← can be deleted
```
...
6. **Follow existing code patterns.** The P/Invoke double-call pattern, the `Task.Run` pattern, the CommunityToolkit.Mvvm `[ObservableProperty]` / `[RelayCommand]` pattern — all established. Copy them.
...
8. **The MSI Summary Information P/Invoke in Task 6 is the hardest part.** The signatures and approach are specified precisely — follow them. The `MsiGetSummaryInformation` / `MsiSummaryInfoGetProperty` / `MsiCloseHandle` trio is the right approach. Do NOT use raw COM IStorage interop.
9. **Window sizing:** The main window is compact (~700×300). Don't make it bigger. PatchCleaner proved that small works.
